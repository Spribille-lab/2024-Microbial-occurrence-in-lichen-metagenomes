#code from https://github.com/lucaz88/genome_comparison_code/blob/main/KM_reconstruction.R
# code used in Zoccarato et al. 2022 https://doi.org/10.1038/s42003-022-03184-4



KMdiagram_fetcher <- function(ncore=1, create_RData=T, path=getwd(), new_date=Sys.Date()) {
 ...
 # plan(multiprocess, workers = ncore)
  fetched_KM <- future_lapply(KM_list, function(imod) { 
    ...
    txt4 <- t(data.frame(strsplit(txt3, '\\" ')))[, c(1,3,5), drop=F]
