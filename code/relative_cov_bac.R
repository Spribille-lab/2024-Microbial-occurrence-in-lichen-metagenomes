...
# 3. Process the data  + exploratory
...
#7. calculate median relative coverage for the four top families
##relatvie to the mycobiont

mags_role_filtered %>% group_by(bac_family) %>% 
  summarize(median_cov=median(relative_cov)) 

mags_role_filtered %>% filter(bac_family %in% c("Acetobacteraceae",
                                                "Acidobacteriaceae",
                                                "Beijerinckiaceae",
                                                "Sphingomonadaceae")) %>%
 # group_by(bac_family) %>% 
...
mags_role_filtered_photo<-mags_role_filtered %>%
   filter(metagenome %in% mags_role$metagenome[grepl("photo",mags_role$confirmed_role)])


#####add column for the depth of coverage of the mycobiont and for the coverage relative to the myco_cov
mags_role_filtered_photo<-mags_role_filtered_photo %>% group_by(metagenome) %>% 
  mutate(photo_cov = ifelse(confirmed_role=="photobiont_cyano",depth_cov,ifelse(confirmed_role=="photobiont_chloro",depth_cov,NA))) %>%
  fill(photo_cov, .direction = 'updown') %>% mutate(relative_cov_photo = depth_cov/photo_cov) %>%
  ungroup()
