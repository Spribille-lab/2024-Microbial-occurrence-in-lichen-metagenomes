...
dna <- readDNAStringSet("analysis/03_metagenome_reanalysis/assembly_SRR14722232.extraction.fasta")
dna <- readDNAStringSet("analysis/03_metagenome_reanalysis/assembly_SRR13125477.bacteria.fasta")
...
assignment %>% filter(grepl("Acetobacteraceae",as.character(assignment)))
assignment %>% filter(grepl("Acidobacteriaceae",as.character(assignment)))
assignment %>% filter(grepl("Beijerinckiaceae",as.character(assignment)))
assignment %>% filter(grepl("Sphingomonadaceae",as.character(assignment)))
assignment %>% filter(grepl("Nostocaceae",as.character(assignment)))
assignment %>% filter(grepl("UBA10450",as.character(assignment))) 
...
write.table(ids,"analysis/03_metagenome_reanalysis/temp_idtaxa/assembly_SRR14722232.txt",sep="\t",quote = F, row.names = F)
